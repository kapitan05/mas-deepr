...
### Gradient mechanics: trajectory tracking → token selection → backprop across the shared LoRA

**Capture.** `_run_agent` captures `resp.messages` (not just `.text`) per call — this holds the full turn sequence for that one `agent.run()` invocation (reasoning text, tool-call JSON, tool results, final answer). Training needs the exact token IDs/logprobs sampled at generation time, not re-tokenized text — decoding then re-encoding can silently produce different token IDs (documented retokenization-drift failure mode; vLLM's OpenAI-compatible API must return `logprobs`, and ART's SDK is built around capturing this directly rather than reconstructing it after the fact).

**Why the loss is a sum over tokens.** For an autoregressive policy, `π_θ(response) = Π_t π_θ(token_t | context_<t)`, so `∇_θ log π_θ(response) = Σ_t ∇_θ log π_θ(token_t | context_<t)` — the gradient of a trajectory's log-prob is literally the sum of each token's own gradient. This is why token-level loss computation isn't an implementation detail, it's what the objective reduces to. GRPO's per-sample loss: `L_i(θ) = A_i · Σ_t log π_θ(token_t | context_<t)` [+ PPO clipping vs `π_θ_old`, + KL penalty vs a frozen reference], where `A_i = (r_i - mean(group)) / std(group)` — no critic network needed, which is exactly why GRPO fits the 1×H100 budget (one fewer resident model vs. PPO).

**Why we mask to specific tokens.** The sum over `t` only includes tokens the policy actually sampled — never prompt tokens, never tool outputs, never retrieved text. `∇_θ log π_θ(token_t)` is only meaningful for a token the model generated from its own distribution; a search result or system prompt token wasn't chosen by `π_θ`, so there's nothing for the gradient to attach to — training on it would push the model to imitate arbitrary external text as if it were a considered decision. Concretely: `loss_mask=1` on the model's own generated segments, `loss_mask=0` on everything fed back in as input. Matches Polar's own mechanism exactly ("copies only sampled assistant tokens as trainable tokens, masks canonical interstitial tokens") — this is a standard agentic-RL requirement, not framework-specific.

**Mapping onto mas-deepr's multi-call, single-LoRA structure.** Manager/Browser*/Synthesizer are separate conversations (different system prompts, no shared autoregressive continuation) — not concatenated into one sequence. Per rollout: `L_rollout(θ) = A_rollout · Σ_{calls c} Σ_{t in c's sampled tokens} log π_θ(token_t | context_<t)`, where `A_rollout` (one outcome reward, group-normalized) is broadcast identically to every call in that rollout since they jointly produced it. Because Manager/Browser/Synthesizer all forward-pass through the *same* shared LoRA, every call's loss term produces gradients on the *same* parameter tensors — ordinary autodiff sums gradient contributions across every reuse of a parameter within a batch; nothing multi-agent-specific is required beyond this. The real open risk (not solved by ART, GRPO, or Polar): broadcasting one reward across a variable number of calls gives a useless browser call the same advantage as a decisive one purely by rollout membership — Polar's own ablation hit exactly this ("noisy credit assignment," real reward hacking observed). Mitigated, not solved, by `rubric_reward.py`'s multi-criterion reward; explicit spot-check for this failure mode belongs at M8.
